...
document.addEventListener("click", (e) => {
  ...
  if (viewBtn) {
    ...
    const price = Number.parseFloat(viewBtn.dataset.price || 0)
    ...
    const defaultSugar =
      document.querySelector('.sugar-btn[data-sugar="Less Sweet"]') || document.querySelector(".sugar-btn")
...
document.addEventListener("click", (e) => {
  ...
  if (addonBtn) {
    ...
    const price = Number.parseFloat(addonBtn.dataset.price || 0)
    ...
    if (addonBtn.classList.contains("active")) {
      modalSelectedToppings[key] = {
        name: addonBtn.querySelector("span")?.textContent.trim() || key,
        price: price,
        qty: 1,
      }
    } else {
    ...
    if (window.recalcModalTotal) window.recalcModalTotal()
      ...
      const base =
        Number.parseFloat((document.getElementById("modalProductPrice")?.textContent || "0").replace(/[^0-9.]/g, "")) ||
        0
      let addons = 0
...
function addToCart(product_id, name, price, size) {
  if (typeof product_id === "object") {
    ...
    const existing = cart.find(
      (it) =>
        it.product_id === itemObj.product_id &&
        it.size === itemObj.size &&
        JSON.stringify(it.toppings || []) === JSON.stringify(itemObj.toppings || []),
    )
    if (existing) existing.quantity += itemObj.quantity || 1
    else cart.push(Object.assign({ quantity: itemObj.quantity || 1 }, itemObj))
  } else {
    const existingItem = cart.find((item) => item.product_id === product_id && item.name === name && item.size === size)
    if (existingItem) {
...
function submitPickupForm(payload) {
  ...
  try {
    console.debug("submitPickupForm payload:", payload)
  } catch (e) {}
  ...
  body.append(
    "payment_method",
    typeof payload.payment_method !== "undefined" && payload.payment_method !== null
  ...
  try {
    console.debug("Submitting to pickup_checkout.php ->", body.toString())
  } catch (e) {}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (gcashDone) {
    ...
    gcashDone.addEventListener("click", (e) => {
      e.preventDefault && e.preventDefault()
      submitGcashCheckout()
    })
  }
...
function showReferenceModal(ref) {
  ...
  const existing = document.getElementById("orderReferenceModal")
  ...
  fetch("login.php", {
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        if (document.getElementById("profileText")) document.getElementById("profileText").textContent = data.firstName
        if (document.getElementById("profileAvatar")) document.getElementById("profileAvatar").innerHTML = data.initials
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (track) {
    ...
    const step = firstItem
      ? firstItem.offsetWidth + (Number.parseInt(getComputedStyle(firstItem).marginRight) || 12)
      : Math.floor(track.clientWidth / 3)
...
async function verifyOTP() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (modalMsg) {
        modalMsg.innerHTML =
          '<div style="color:#10B981;font-weight:bold;margin:10px 0;"><i class="fas fa-check-circle"></i> Verification successful!</div>'
      }
    ...
    .then((data) => {
      ...
      if (Array.isArray(data.all_recent_orders)) {
        data.all_recent_orders.forEach((order) => {
          ...
          if (initialLoadComplete && previousOrderStatuses[ref] && previousOrderStatuses[ref].toLowerCase() !== cur) {
            const key = `${ref}:${cur}`
...
function getStatusMessage(status, location, time) {
  switch (status.toLowerCase()) {
    ...
    case "ready":
      return "Your order is ready for pickup at " + (location || "our store") + (time ? " at " + time : "") + "."
    case "completed":
  ...
  if (typeof window.PHP_IS_LOGGED_IN !== "undefined" && window.PHP_IS_LOGGED_IN) {
    ...
    if (window.PHP_USER_FULLNAME) {
      currentUser = {
        ...
        initials: window.PHP_USER_FULLNAME.split(" ")
          .map((n) => n[0])
          .join("")
          .toUpperCase()
          .substring(0, 2),
...
async function loadActiveToppings() {
  try {
    ...
    container.innerHTML = data.toppings
      .map((t) => {
...
document.addEventListener("click", (e) => {
  const sel = e.target
  if (
    sel.closest(".btn-view") ||
    sel.closest(".view-product") ||
    sel.closest("[data-open-product]") ||
    sel.classList.contains("product-view-btn")
  ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      if (!name) {
        resultEl.textContent = "Name required"
        return
      }
  ...
  try {
    ...
    document.querySelectorAll(".topping-checkbox").forEach((cb) => {
      cb.onchange = (e) => {
        ...
        const price = Number.parseFloat(cb.getAttribute("data-price")) || 0
        if (cb.checked) {
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (registerLastName) {
    registerLastName.addEventListener("blur", () => {
