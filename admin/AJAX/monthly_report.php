...
try {
    ...
    if ($type !== '') {
                $totSql = "SELECT COUNT(DISTINCT t.transac_id) orders,
                     ...
                     $locJoin
                                     WHERE DATE(CONVERT_TZ(t.created_at, @@session.time_zone, '+00:00')) BETWEEN ? AND ?
    ...
    if ($type !== '') {
                                $dailySql = "SELECT DATE(CONVERT_TZ(t.created_at, @@session.time_zone, '+00:00')) d,
    ...
    $dailySql .= " GROUP BY DATE(CONVERT_TZ(t.created_at, @@session.time_zone, '+00:00')) ORDER BY d ASC";
